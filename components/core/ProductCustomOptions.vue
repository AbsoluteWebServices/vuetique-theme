  ...
  <form class="custom-options mt-5">
    <div v-for="option in product.custom_options" :key="('customOption_' + option.option_id)">
      <div class="custom-option mb-4">
        ...
        >
        <template v-if="option.type === 'radio' || option.type === 'select'">
          <div class="checkbox-wrap" v-for="opval in option.values" :key="opval.option_type_id">
            <input
              ...
              v-model="inputValues[('customOption_' + option.option_id)]"
            ><label class="pl-3 lh-5 h4 pointer" :for="('customOption_' + option.option_id + '_' + opval.option_type_id)" v-html="opval.title" />
          </div>
        </template>
